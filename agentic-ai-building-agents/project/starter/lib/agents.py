...
class Agent:
    def __init__(self, 
                 model_name: str,
                 instructions: str,
                 openai_api_key: Optional[str] = None, # UDA key to be passed to LLM
                 response_format: Optional[TypeVar('BaseModel')] = None, # UDA response format for final answer
                 tools: List[Tool] = None,
                 evaluation_tool: Optional[Tool] = None, # UDA evaluation tool to assess retrieved docs
    ...
    def _llm_step(self, state: AgentState) -> AgentState:
        ...
        llm = LLM(
            ...
            tools=self.tools, # UDA available tools to execute user requests
            api_key=self.openai_api_key,
        )

        # UDA get messages since latest user query
        messages_since_last_user_query = []
        for msg in reversed(state["messages"]):
            messages_since_last_user_query.insert(0, msg)
            if isinstance(msg, UserMessage):
                break
        
        # UDA get the latest tool message since most recent user query
        latest_tool_message = None
        stop_iteration = False
        for msg in messages_since_last_user_query:
            if isinstance(msg, ToolMessage):
                latest_tool_message = msg
                break
        
        if latest_tool_message:
            # UDA evaluate the retrieved documents using the evaluation tool
            evaluation = self.evaluation_tool(
                question=state["user_query"],
                retrieved_docs=json.loads(latest_tool_message.content)
            )
            parsed_evaluation = json.loads(evaluation)

            # UDA if documents are useful, generate final answer
            if bool(parsed_evaluation['useful']):
                final_llm = LLM(
                    model=self.model_name,
                    temperature=self.temperature,
                    api_key=self.openai_api_key,
                ...
                final_message = SystemMessage(
                    content="Provide an answer based on the following description: "
                            f"{parsed_evaluation['description']}. "
                            "List the name of tools from {self.tools} "
                            "that were used in the {messages_since_last_user_query} "
                            "to generate the answer. "
                            "Cite your sources appropriately with urls if applicable."
                )
                final_messages = state["messages"] + [final_message]
                response = final_llm.invoke(final_messages, response_format=self.response_format)
                tool_calls = None
                stop_iteration = True
        
        # UDA if no useful documents, continue normal LLM processing
